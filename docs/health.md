...
`auth/oidc`, `auth/opa`, `plugins` ship one) and registered into a shared `Coordinator`. The coordinator never blocks on a check longer than
`WithCheckTimeout`; results live in a sharded cache, so probe handlers stay cheap under load.
...
Full reference: [`config/health.go`](../config/health.go).
...
The factory in [`observability/health/factory`](../observability/health/factory) maps every field above onto its `With*` counterpart. `healthCheckInterval`
...
| Subsystem            | File                                          |
|----------------------|-----------------------------------------------|
| `auth/oidc`          | [`auth/oidc/health.go`](../auth/oidc/health.go)         |
| `auth/opa`           | [`auth/opa/health.go`](../auth/opa/health.go)           |
| `security/secrets`   | [`security/secrets/health.go`](../security/secrets/health.go) |
| `security/vault`     | [`security/vault/health.go`](../security/vault/health.go) |
| `plugins`            | [`plugins/health.go`](../plugins/health.go)             |
