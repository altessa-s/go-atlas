...
// of the unary interceptor on the happy path (client IP in the allowlist).
func BenchmarkUnaryInterceptor_Allowed(b *testing.B) {
	inter := ServerUnaryInterceptor(newTestRegistry())
...
// the method is on the ignore list and access checks are skipped entirely.
func BenchmarkUnaryInterceptor_IgnoredMethod(b *testing.B) {
	inter := ServerUnaryInterceptor(newTestRegistry(), WithIgnoreMethods("/test.Service/Denied"))
