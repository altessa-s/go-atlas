...
// The benchmarks use the in-process mockResolver from geoacl_test.go instead
// of a real GeoIP database, so they measure the middleware passthrough cost
// (path interning, ignore filtering, registry evaluation) without lookup I/O.

func BenchmarkHandler_Allowed(b *testing.B) {
	handler := Middleware(newTestResolver(), newTestRegistry())(nopHandler)
...
func BenchmarkHandler_IgnoredPath(b *testing.B) {
	mw := New(newTestResolver(), newTestRegistry(), WithIgnorePaths("/skip"))
