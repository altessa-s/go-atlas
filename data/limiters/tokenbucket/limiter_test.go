...
func TestNew_PanicsOnNilConfig(t *testing.T) {
	...
	tokenbucket.New(nil, memory.New())
}

func TestNew_PanicsOnInvalidConfig(t *testing.T) {
	defer func() {
		if r := recover(); r == nil {
			t.Error("New(invalid, ...) should panic")
		}
	}()
	tokenbucket.New(&tokenbucket.RateLimitConfig{}, memory.New())
}

func TestNew_ValidConfig(t *testing.T) {
	l := tokenbucket.New(validConfig(), memory.New())
	if l == nil {
...
func TestLimit_IPBased_DefaultRule(t *testing.T) {
	cfg := validConfig()
	l := tokenbucket.New(cfg, memory.New(),
