...
// limiterMetrics holds all Prometheus metrics for the RuleLimiter.
...
func newLimiterMetrics(c metrics.Collector) *limiterMetrics {
	...
	scoped := c.WithSubsystem("rate_limiter")
	...
	return &limiterMetrics{
		...
		limitCheckErrors: scoped.MustCounter(metrics.MetricOpts{
			...
			Help: "Total number of errors during rate limit checks.",
		}),
	}
}
