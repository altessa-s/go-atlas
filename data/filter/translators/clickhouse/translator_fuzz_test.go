...
func newTranslator(tb testing.TB) *clickhouse.Translator {
	tb.Helper()

	tr, err := clickhouse.NewTranslator(
...
//
// The scanner below models this dialect specifically: ClickHouse backslash-escapes both the quote and the backslash. The shared walker
...
// endsOutsideLiteral walks generated ClickHouse SQL and reports whether the
// text ends outside a string literal.
//
// ClickHouse quotes with single quotes and escapes with a backslash, so a
// backslash consumes whatever follows it and only an unescaped quote toggles
// the state. A caller value that escaped its quoting leaves the scanner inside
// a literal at the end of the clause, or turns the tail into SQL that was meant
// to be data.
...
// '?' is data, not a marker, and counting it would make the argument tally
// disagree for a perfectly correct clause.
