...
// Translator converts filter AST nodes to PostgreSQL WHERE clauses.
//
// Translate emits numbered `$n` placeholders and returns the bound
// values alongside the clause; TranslateInline emits self-contained SQL
// with every literal rendered in place. Both are promoted from the
// shared SQL walker — see [sqlbase.Translator].
//
...
// NewTranslator creates a new PostgreSQL translator with the given
